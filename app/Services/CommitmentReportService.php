...
class CommitmentReportService
{
    ...
    public function report(?array $entityIds = null, ?string $status = 'Active'): array
    {
        ...
        $rows = $commitments->map(function (Commitment $commitment) {
            return [
                ...
                'settling_within_30' => $commitment->settlement_date
                    && $commitment->settlement_date->lte(now()->addDays(30)),
                'settling_within_90' => $commitment->settlement_date
                    && $commitment->settlement_date->lte(now()->addDays(90)),
            ];
        });
        ...
        return [
            ...
            'totals' => $totals,
            'by_type' => $this->groupTotals($activeRows, 'commitment.commitment_type'),
            'by_entity' => $this->groupTotals($activeRows, 'entity_name'),
     ...
     */
    private function groupTotals(Collection $rows, string $key): array
    {
