...
- **Loan account** owns loan economics: `loan_interest`, `loan_fees`, `loan_repayments`, plus `director_loan_in` / `director_loan_out` when the director funds or redraws on that ledger. Treat its CSV as loan activity, not cash reconciliation. Loan-ledger transactions must not post to Bank/Cash (`1100`).
...
- Loan-purpose import UI uses `Transaction::loanActivityTypeSelectGroups()` (loan activity + director loan in/out) and “Update loan activity”. Do not mix loan pending lines into operating cash unmatched totals. Surface unmatched cash-side gaps via `UnmatchedLoanRepaymentAuditor` on the loan panel.
