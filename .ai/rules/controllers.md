...
Manual invoice lines accept negative unit_price (fee/deduction lines that reduce the total). Do not re-add min:0 on lines.*.unit_price or min="0" on the unit price input. InvoicePostingService flips signed amounts to non-negative debit/credit sides (negative income/GST becomes a debit). Line account_code remains income-only for now.
