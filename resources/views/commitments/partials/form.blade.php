<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        ...
        <select name="commitment_type" id="commitment_type" required class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-600">
            @foreach(\App\Models\Commitment::TYPES as $type)
                <option value="{{ $type }}" {{ old('commitment_type', $commitment->commitment_type ?? 'Property') === $type ? 'selected' : '' }}>{{ $type }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input type="number" step="0.01" min="0" name="contract_price" id="contract_price" required
               value="{{ old('contract_price', $commitment->contract_price ?? '') }}"
...
<div>
    ...
    <input type="text" name="name" id="name" required
           value="{{ old('name', $commitment->name ?? '') }}"
           class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-600">
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        ...
        <input type="date" name="contract_date" id="contract_date"
               value="{{ old('contract_date', isset($commitment) && $commitment->contract_date ? $commitment->contract_date->format('Y-m-d') : '') }}"
               class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-600">
    ...
    <div>
        ...
        <input type="date" name="settlement_date" id="settlement_date"
               value="{{ old('settlement_date', isset($commitment) && $commitment->settlement_date ? $commitment->settlement_date->format('Y-m-d') : '') }}"
               class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-600">
...
<div>
    ...
    <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-900 dark:border-gray-600">{{ old('notes', $commitment->notes ?? '') }}</textarea>
