...
>
    @if ($isClosed)
        <div class="mb-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-900 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-100" role="status" data-closed-notes-notice>
            <p class="font-medium">{{ __('Notes cannot be changed') }}</p>
            <p class="mt-1 text-rose-800 dark:text-rose-200">
                {{ __('This entity is closed. Reopen it from Edit company profile (set Status to Active) before adding or deleting notes.') }}
            </p>
        </div>
    @endif
