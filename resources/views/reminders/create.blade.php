        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('reminders.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500" required></textarea>
                    @error('content')
                ...
                <div>
                    ...
                    <x-date-input name="reminder_date" id="reminder_date" value="{{ old('reminder_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500" min="{{ date('Y-m-d') }}" required />
                    @error('reminder_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="repeat_type" id="repeat_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div id="repeat_end_date_container" style="display: none;">
                    ...
                    <x-date-input name="repeat_end_date" id="repeat_end_date" value="{{ old('repeat_end_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500" min="{{ date('Y-m-d') }}" />
                ...
                <div class="flex justify-end space-x-4">
                    <a href="{{ url()->previous() }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-xs text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Cancel
                    </a>
                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-xs text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Create Reminder
                    </button>
                </div>
            </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            repeatTypeSelect.addEventListener('change', function() {
                const showEndDate = this.value !== 'none';
                repeatEndDateContainer.style.display = showEndDate ? 'block' : 'none';
                if (showEndDate) {
                    window.redrawFlatpickr?.(repeatEndDateContainer);
                }
            });
        });
    </script>
