    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('business-entities.index') }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-md transition-all duration-200 ease-in-out transform hover:scale-105">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if ($assets->isEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mb-6">Add an asset from a business entity to see it here.</p>
                    <a href="{{ route('business-entities.index') }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-md transition-all duration-200 ease-in-out transform hover:scale-105">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                            ...
                            <div class="p-6">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if($asset->businessEntity)
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate" title="{{ $asset->businessEntity->legal_name }}">
                                                {{ $asset->businessEntity->legal_name }}
                                            </p>
                                        @endif
                                    </div>
                                    <div class="flex space-x-2 flex-shrink-0">
                                        <a href="{{ route('business-entities.assets.edit', [$entityId, $asset->id]) }}" class="text-gray-400 hover:text-indigo-600 transition-colors">
                                ...
                                <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('business-entities.assets.show', [$entityId, $asset->id]) }}" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-center py-2 px-3 rounded-lg text-sm font-medium transition-colors">
                                            View Details
                                        </a>
                                        <a href="{{ route('business-entities.assets.edit', [$entityId, $asset->id]) }}" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 text-center py-2 px-3 rounded-lg text-sm font-medium transition-colors">
                                            Edit
                                        </a>
                                    </div>
                                </div>
