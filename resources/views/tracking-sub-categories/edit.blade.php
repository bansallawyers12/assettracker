...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white shadow-xs sm:rounded-lg">
            <form action="{{ route('business-entities.tracking-categories.tracking-sub-categories.update', [$businessEntity, $trackingCategory, $trackingSubCategory]) }}" method="POST" class="px-4 py-5 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" 
                               name="name" 
                               id="name" 
                               value="{{ old('name', $trackingSubCategory->name) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  id="description" 
                                  rows="3"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-xs focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $trackingSubCategory->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="sort_order" 
                               id="sort_order" 
                               value="{{ old('sort_order', $trackingSubCategory->sort_order) }}"
                               min="0"
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" 
                               ...
                               id="is_active" 
                               value="1"
                               {{ old('is_active', $trackingSubCategory->is_active) ? 'checked' : '' }}
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('business-entities.tracking-categories.show', [$businessEntity, $trackingCategory]) }}" 
