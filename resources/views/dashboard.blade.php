<x-app-layout>
    <div class="py-6 lg:py-8 bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @php
                $dashboardSearchIndex = $businessEntities->map(function ($e) {
                    return [
                        'type'  => 'entity',
                        'label' => $e->legal_name ?? '',
                        'sub'   => $e->entity_type ?? '',
                ...
                })->concat($assets->map(function ($a) use ($businessEntities) {
                ...
                }))->concat($uniquePersons->map(function ($pd) {
                    $person = $pd['person'];
            ...
            @endphp
            <script type="application/json" id="dashboard-search-index-data">@json($dashboardSearchIndex)</script>

            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-5" id="dashboard-entity-asset-search-wrap">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-blue-500 dark:text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <h2 class="text-sm font-bold text-gray-900 dark:text-white">Quick Search</h2>
                    </div>
                    <p class="text-xs text-gray-400 dark:text-gray-500">Entities · Assets · Persons &nbsp;&mdash;&nbsp; use <kbd class="px-1.5 py-0.5 text-[10px] font-mono border border-gray-200 dark:border-gray-600 rounded">↑↓</kbd> <kbd class="px-1.5 py-0.5 text-[10px] font-mono border border-gray-200 dark:border-gray-600 rounded">Enter</kbd> to navigate</p>
                </div>
                <label for="dashboard-entity-asset-search" class="sr-only">Search entities, assets and persons</label>
                <div class="relative">
                    <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <input
                        type="text"
                        id="dashboard-entity-asset-search"
                        autocomplete="off"
                        spellcheck="false"
                        ...
                        class="w-full pl-9 pr-16 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/50 text-gray-900 dark:text-white placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 transition"
                    >
                    <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                        <span id="dashboard-search-count" class="hidden text-[11px] font-medium text-gray-400 dark:text-gray-500 tabular-nums"></span>
                        <button type="button" id="dashboard-search-clear" class="hidden p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200/60 dark:hover:bg-gray-700 transition" aria-label="Clear search">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                        </button>
                    </div>
                    <div
                        id="dashboard-search-results"
                        class="hidden absolute z-50 left-0 right-0 mt-1.5 max-h-80 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-xl divide-y divide-gray-100 dark:divide-gray-700"
                        role="listbox"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            (function initDashboardEntityAssetSearch() {
                const wrap     = document.getElementById('dashboard-entity-asset-search-wrap');
                const input    = document.getElementById('dashboard-entity-asset-search');
                const panel    = document.getElementById('dashboard-search-results');
                ...
                const dataEl   = document.getElementById('dashboard-search-index-data');
                if (!wrap || !input || !panel || !dataEl) return;

                let index = [];
                try { index = JSON.parse(dataEl.textContent); } catch (_) { return; }
                ...
                function setActive(idx) {
                    ...
                    items.forEach((el, i) => {
                        ...
                        el.classList.toggle('dark:bg-gray-700/80', i === idx);
                        if (i === idx) {
                ...
                function render(query) {
                    ...
                    const matches = index.filter((item) =>
                        ((item.label || '') + ' ' + (item.sub || '')).toLowerCase().includes(ql)
                    ).slice(0, 30);

                    if (countEl) {
                        countEl.textContent = matches.length > 0 ? matches.length + (matches.length === 30 ? '+' : '') : '';
                        countEl.classList.toggle('hidden', matches.length === 0);
                    }

                    if (matches.length === 0) {
                        panel.innerHTML = '<div class="flex items-center gap-2 px-4 py-4 text-sm text-gray-400 dark:text-gray-500"><svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>No results for <strong class="text-gray-600 dark:text-gray-300 font-semibold">' + q.replace(/</g, '&lt;') + '</strong></div>';
                        openPanel();
                        return;
                    }
