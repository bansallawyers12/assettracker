...
@if($report['is_consolidated'] ?? false)
    <div class="px-6 py-3.5 text-xs leading-relaxed border-b border-amber-200 bg-amber-50 text-amber-950" data-consolidated-report-notice>
        <p class="font-semibold text-amber-900">
            Consolidated {{ $reportType }} — sum of entity journals (no intercompany elimination)
        </p>
        <ul class="mt-1.5 list-disc pl-4 space-y-0.5 text-amber-900/90">
            <li data-consolidated-no-elimination>
                Totals <strong>add</strong> each entity’s posted journals. Intercompany Director / Entity Loan (2500) and related-party legs are <em>not</em> auto-eliminated — the same loan can appear on both sides and inflate consolidated figures unless they net out.
            </li>
            <li>Select <strong>one entity</strong> in Entity scope above for a single-entity {{ strtolower($reportType) }}.</li>
            @if(($report['entity_breakdown']['entities'] ?? collect())->isNotEmpty())
