<div class="p-5 sm:p-6">
    ...
    @if (! $user->canBeDeleted())
        <div class="mt-6 rounded-xl border border-amber-200/80 bg-amber-50/50 p-4 dark:border-amber-900/40 dark:bg-amber-950/20">
            <p class="text-sm text-amber-900 dark:text-amber-200">
                {{ $user->deleteBlockedReason() }}
            </p>
            <p class="mt-2 text-sm text-amber-800 dark:text-amber-300">
                {{ __('Ask an administrator to reassign your entities or deactivate your account instead.') }}
            </p>
        </div>
    @else
        <div class="mt-6 rounded-xl border border-red-200/80 bg-red-50/50 p-4 dark:border-red-900/40 dark:bg-red-950/20">
            <p class="text-sm text-red-800 dark:text-red-200">
                {{ __('Before deleting your account, download or export any personal information you need to keep. This cannot be undone.') }}
            </p>
            ...
            <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Enter your password to confirm. Portfolio entities and accounting records will not be deleted.') }}
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <x-input-error :messages="$errors->userDeletion->get('userDeletion')" class="bank-field-error mt-1.5" />
                </div>
