...
| ID | Type | Sev | Summary | User impact | Evidence | Status |
| --- | --- | --- | --- | --- | --- | --- |
| entity-001 | UX | Medium | Tenancy / property-manager contacts (`exclude_from_financial_reports`) | P&L, bank, journals, officer roles hidden; users may not understand missing tabs | `EnsuresOperationalBusinessEntity`, `tests/Feature/EntityAccountingNavTest.php` | **Fixed** — banner lists missing sections (incl. invoices/bank) + convert CTA; accounting nav chip; Persons/invoices/bank tab notices; Contact only badges |
| entity-002 | Functionality | Medium | Closed entities block mutations (403) | Bank links, transactions, edits fail on closed entities | `EnsuresOperationalBusinessEntity::ensureNotClosed` | **Fixed** — closed banner + reopen CTA; hide mutate buttons (contacts/notes/docs/compliance/invoices/bank); JSON 403 message; compliance workspace forced locked; profile “Save & reopen” |
