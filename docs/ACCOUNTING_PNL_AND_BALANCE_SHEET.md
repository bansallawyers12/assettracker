...
6b. **Bank-received income is not a 2500 receivable.** Account-transactions reconstruction skips bank-received operating income even when `paid_by` is another entity (`isBankReceivedOperatingIncomeForDirectorLoanReport`). Cross-entity Bank/Cash on the booker is omitted in favour of the payer’s `TXN-…-PAY` (or a synthetic payer amount). The sheet can therefore look balanced without an intercompany 2500 receivable — intentional; BS notices state this. Trust posted 2500 GL for the loan balance.

6c. **Cash flow is GL-category movement, not a cash roll-forward.** Operating/investing/financing sections sum period journal movement on income/expense, fixed assets, and liabilities. Capitalised loan interest/fees (Dr P&L / Cr 4000) appear in those sections without moving 1100; cash↔cash transfers that share 1100 are invisible. The Cash Flow report banner explains this.
...
14. Consolidated P&L/BS simply **sums entity journals**. Intercompany 2500 legs can remain on a consolidated sheet unless they net out; they are not auto-eliminated as a group. The consolidated report banner (`data-consolidated-no-elimination`) states this — pick a single entity for entity-level figures.
